...
grep -Fq 'migrate.migrationsExecuted=3' <<<"$migrate_first" \
...
grep -Fq 'info.current=3' <<<"$info_after" || ci_fail "empty_info_current_after_unexpected"
...
ci_assert_equals "23" "$business_tables" "empty_business_table_count_unexpected"
ci_assert_equals "24" "$all_tables" "empty_total_table_count_unexpected"
...
ci_assert_equals "3" "$history_total" "empty_history_row_count_unexpected"
