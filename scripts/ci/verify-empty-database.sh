...
grep -Fq 'migrate.migrationsExecuted=2' <<<"$migrate_first" \
...
grep -Fq 'info.current=2' <<<"$info_after" || ci_fail "empty_info_current_after_unexpected"
...
ci_assert_equals "22" "$business_tables" "empty_business_table_count_unexpected"
ci_assert_equals "23" "$all_tables" "empty_total_table_count_unexpected"
ci_assert_equals "1" "$history_rows" "empty_v1_history_missing"
ci_assert_equals "1" "$v2_history_rows" "empty_v2_history_missing"
ci_assert_equals "2" "$history_total" "empty_history_row_count_unexpected"
