...
grep -Fq 'migrate.migrationsExecuted=5' <<<"$migrate_first" \
...
grep -Fq 'info.current=5' <<<"$info_after" || ci_fail "empty_info_current_after_unexpected"
...
ci_assert_equals "5" "$history_total" "empty_history_row_count_unexpected"
