...
@Component
public class QueryOverdueTasksAgentTool implements AgentTool<EmptyToolArguments> {
    private final AgentReadMapper readMapper;
    private final PermissionService permissionService;
    private final AgentProperties properties;

    public QueryOverdueTasksAgentTool(AgentReadMapper readMapper,
    ...
    public Object execute(ToolExecutionContext context, EmptyToolArguments arguments) {
        ...
        return readMapper.selectOverdueProjectTasks(context.projectId(), LocalDate.now(),
                        properties.getProjectTaskLimit()).stream()
                .map(row -> new ProjectTaskToolItem("O" + sequence.incrementAndGet(), row.getTitle(),
