-- STAGE B ONLY: main-database rollback script.
-- Requires the same explicit session authorization as the apply script.
-- It only rolls back the batch whose project deleted_at/update_time marker
-- matches the target rows and whose task update_time matches that marker.
...
SET @guard_sql = IF(
    ...
    'SELECT * FROM __stage0_abort_main_rollback_candidate_mismatch__'
);
PREPARE guard_stmt FROM @guard_sql;
EXECUTE guard_stmt;
DEALLOCATE PREPARE guard_stmt;
...
SET @guard_sql = IF(
    ...
    'SELECT * FROM __stage0_abort_main_rollback_affected_row_mismatch__'
);
PREPARE guard_stmt FROM @guard_sql;
EXECUTE guard_stmt;
DEALLOCATE PREPARE guard_stmt;
...
SELECT 'main repair rolled back' AS result,
