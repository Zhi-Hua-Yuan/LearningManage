# 阶段 2 风险登记表

状态：`WP6 LOCAL PASS / STAGE FROZEN`
建立日期：2026-09-04

| ID | 状态 | 风险 | 触发条件 | 缓解措施 | 目标工作包 |
|---|---|---|---|---|---|
| S2-R-001 | CLOSED | 阶段 2 分支未从包含阶段 1 最终证据的受保护 develop 创建 | 本地分支不是最新受保护基线 | WP3 收口分支从合并后 `develop=e76e2b5` 创建，阶段 1 候选 `5057158` 仍为当前 HEAD 祖先 | WP0/WP1 |
| S2-R-002 | CLOSED | 部分场景绕过 Pipeline 直接调用模型 | 静态扫描发现新增或遗留直接调用，或 MySQL 终态测试失败 | WP3 已迁移全部生产调用；精确 ArchUnit、源码扫描、4 个 MySQL 集成测试及合并后 Backend CI run 33867556286 全部通过，业务直连模型调用为 0 | WP3 |
| S2-R-003 | OPEN | Qwen/OpenAI-compatible 响应对 Tool Calls、Usage 或请求 ID 的格式差异 | 真实模型与 Stub 字段不一致 | WP2 已完成供应商隔离、16 场景 Stub、真实 HTTP 两轮往返和协议失败分类；WP7 继续真实受控验证 | WP7 |
| S2-R-004 | CLOSED | 同一草稿使用不同 operationId 重复落库 | 旧确认日志唯一约束不足 | WP5 已实现草稿行锁、草稿级重放、Handler 权限重验、CAS 和单事务写入；候选 run `33889047346` 的 20 路 MySQL 并发确认只产生一次正式写入，重排竞争与原子回滚同时通过 | WP5 |
| S2-R-005 | OPEN | AI 请求/响应日志泄漏私人内容或凭据 | 脱敏发生在持久化之后 | WP6 已实现统一脱敏、先哈希后截断和唯一日志写入口；等待候选 Gitleaks、MySQL 与产物扫描后关闭 | WP6 |
| S2-R-006 | OPEN | 重试和模型回退造成不可控成本 | 网络抖动、429 或上游 5xx | WP6 已限制主/兜底各一次并按实际模型聚合 Usage/成本；等待候选故障注入后关闭 | WP3/WP6 |
| S2-R-007 | OPEN | 熔断或 Bulkhead 配置错误阻塞核心业务 | 外部模型不可用或并发耗尽 | WP6 已实现全局 Semaphore Bulkhead、模型级熔断、总期限和启动校验；等待候选 Docker/CI 后关闭 | WP6 |
| S2-R-008 | OPEN | 场景拆分导致旧 API 或前端行为回归 | 门面签名、响应字段或错误语义变化 | 候选 run `33875362611` 在后端合并 SHA `d3a9c673` 上通过 653 项后端验证、Docker 全栈、运行时 OpenAPI 44/44 匹配和 AI Stub 闭环；按计划保留至 WP8 最终跨仓兼容验收后关闭 | WP4/WP7/WP8 |
| S2-R-009 | OPEN | AI 日志查询暴露系统管理员不应读取的项目内容 | 运维查询绕过资源权限 | WP6 已加入 Mapper 依赖静态阻断且所有详情按当前用户过滤；等待候选架构门禁后关闭 | WP6/WP8 |

## 使用规则

- `OPEN` 风险必须在目标工作包关闭、降级为明确限制或进入阶段验收的已接受风险。
- 不删除历史风险 ID，不把未验证项写成 PASS。
- 本文件不保存密码、Token、API Key、完整 Prompt 或业务正文。
