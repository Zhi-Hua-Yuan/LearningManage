...
该提交作为 PR4 业务实现完成基线。D2-F 文档收口提交合并后，新的 `develop` SHA
应在本记录的收口 PR 中补充；不得把文档收口提交与 PR #49 的业务实现提交混为一谈。
...
D2-E 验收记录保存了 CI run `33239260276` 和隔离 MySQL 的完整结果。若合并后另有
针对 `develop` 的 CI run，应在提交 D2-F 合并收口 PR 时追加其编号和链接；不得伪造
或用本地未配置数据库凭据的结果替代受保护 CI。
...
- [ ] D2-F 收口 PR 合并后补录最终 `develop` SHA；
- [ ] 收口 PR 合并后的 CI 链接补录（若与 D2-E CI run 不同）。

以上两项是 D2-F 文档 PR 合并后的事实记录，不是业务实现缺口。完成该收口 PR
并通过其合并后 CI 后，本记录状态才能由 `READY_FOR_MERGE` 改为
`PASS / COMPLETED`。
