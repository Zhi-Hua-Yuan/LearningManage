...
状态：阶段 1 执行中；PR5 已完成；当前主目标为 PR6
...
| S1-R-008 | OPEN | 普通接口已鉴权但 AI 入口可绕过 | AI 请求携带越权 projectId/taskId | C4 已补齐自动候选过滤、显式 ID 全量校验、周复盘润色草稿确认重授权；待 CI MySQL/完整门禁后关闭 | AI 越权回归测试 | PR6 |
...
| S1-R-012 | OPEN | 创建人与受理人口径混用导致统计错误 | 周复盘统计仍按 `task.user_id` 查询执行者 | C4 已将周复盘完成数/重点项目统一改为 `assignee_user_id`，并新增 Mapper SQL 合同与 MySQL 回归；待 CI 数据库证据后关闭 | 周统计回归测试 | PR6 |
