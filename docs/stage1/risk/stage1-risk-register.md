...
状态：阶段 1 执行中；PR6 已完成；当前主目标为 PR7
...
| S1-R-004 | CLOSED | TEAM 周复盘泄漏私人正文 | 复用实体或同一 VO 返回 | DTO/VO 分离；共享 VO 类型不含私人字段 | PR6 C5：共享 VO JSON 脱敏、MySQL 共享查询排除 PRIVATE、Backend CI run 33348660533 | PR6 |
...
| S1-R-008 | CLOSED | 普通接口已鉴权但 AI 入口可绕过 | AI 请求携带越权 projectId/taskId | C4 已补齐自动候选过滤、显式 ID 全量校验、周复盘润色草稿确认重授权 | PR6 C5 AI 授权回归；越权任务模型调用次数为 0；Backend CI run 33348660533 | PR6 |
...
| S1-R-012 | CLOSED | 创建人与受理人口径混用导致统计错误 | 周复盘统计仍按 `task.user_id` 查询执行者 | C4 已将周复盘完成数/重点项目统一改为 `assignee_user_id`，并新增 Mapper SQL 合同与 MySQL 回归 | PR6 C5/C4 周统计真实 MySQL 回归；Backend CI run 33348660533；554 项测试通过 | PR6 |
